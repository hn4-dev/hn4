...
Legacy Copy-on-Write (CoW) filesystems (e.g., ZFS, Btrfs) rely on pointer trees. To modify a data block atomically, they must allocate a new block, write the data, and then propagate the pointer update up the tree to the root. This introduces a **"Tree Tax"**: a single 4KB data write can trigger 16KB+ of metadata writes and multiple non-sequential I/O operations.

**HN4 eliminates pointer chains.** It utilizes **Ballistic Equations**.
To modify data, the engine does not update a pointer; it solves for a new trajectory coordinate.

**The Shadow Hop** is the mechanism by which data moves to a new physical location without requiring a metadata tree rewrite. It exploits the multi-variate nature of the HN4 addressing equation to create deterministic "Orbital Shells" ($k$) that serve as atomic versioning slots.

---

## 2. Mathematical Foundation

The physical location of any block is derived from the immutable parameters stored in the file's **Anchor** (Spec Section 6.1):

$$ LBA_{phys} = LBA_{base} + \left( [ G + (N \times V \times 2^M) + \Theta(k) ] \pmod{\Phi} \right) $$

*   **$G$ (Gravity Center):** The physical start index (seed).
*   **$N$ (Sequence):** The logical block offset.
*   **$V$ (Vector):** The stride/velocity factor.
*   **$M$ (Scale):** The block size exponent ($2^M$).
*   **$\Theta(k)$:** The Collision/Version offset function.

In a resting state, data resides at the **Primary Orbit ($k=0$)**.
When modifying data, the engine does not overwrite $k=0$. Instead, it solves for the **Shadow Orbit ($k=1$)**.

### Bounded Search Window
The reader probes $k$ within a strict window (default $k \in \{0..12\}$).
*   **Standard Operation:** Files oscillate deterministically between $k=0$ and $k=1$.
...
*   It reads the block header to verify `Generation == Anchor.write_gen`.
*   This identifies the "Old Data" that will be eclipsed.

### PHASE 2: The Hop (Allocation)
The driver calculates the trajectory for **$k=1$**.
*   It checks the in-memory **Void Bitmap**.
*   If $k=1$ is free, it is reserved. If occupied, it probes $k=2$, and so on.
...
The driver issues a **Force Unit Access (FUA)** or `NVMe Flush` command.
*   **Invariant:** The Anchor Update (Phase 5) is *never* issued until the storage controller acknowledges that the Shadow Write is durable on non-volatile media.
...
*   **Action:** Updates the in-memory bitmap (Clear Bit) and optionally issues a TRIM/Discard command.
*   **Failure Mode:** If power fails here, the old block remains allocated but "stale" (Gen 100 < Anchor 101). It is effectively garbage, cleaned up later by the Scavenger process.

---

## 4. Crash Consistency Model

Shadow Hop guarantees **Linearizable Atomicity**. There is no intermediate state where a partial write or corrupted block is visible to the system.

| Event | Time $T$ | Block N State | Global File State | Power Failure Result |
| :--- | :--- | :--- | :--- | :--- |
| **Start** | 0 | $k=0$ (Gen 100) | Gen 100 | File Valid (Old Version) |
| **Shadow Write** | 1 | $k=1$ (Gen 101) | Gen 100 | File Valid (Old Version). Shadow block ignored. |
...
2.  **Integrity:** `CRC32` checksum matches.
3.  **Freshness:** `Header.generation` == `Anchor.write_gen`.

**Logic:** Reads reject any block where `Gen < Anchor.Gen`. Writes only increment `Gen`. This prevents stale reads (Phantom Blocks) without requiring locks.
...
## 6. Architecture Comparison: CoW vs. Shadow Hop
...
| **Garbage Collection**| Complex. Reference counting required. | Instant. "Eclipse" frees space immediately. |

---

## 7. Orbital Recurrence (The "Toggle" Effect)

The system does not consume infinite $k$ slots. It actively recycles them.
...
**Engineering Consequence:** A file undergoing heavy random I/O "toggles" between two mathematical coordinates ($LBA_X$ and $LBA_Y$). It does not scatter data arbitrarily across the drive unless those primary slots are physically blocked by other files. This behavior is FTL-friendly, as it presents a stable logical-to-physical mapping pattern to the SSD firmware.
