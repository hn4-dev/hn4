...
/*
 * Test 18.1: Refcount Busy State
 * RATIONALE:
 * Ensures the unmount operation is atomic relative to open handles.
 * If ref_count > 1 (meaning a handle is still open + the volume itself),
 * unmount must reject the request to prevent Use-After-Free.
 */
hn4_TEST(Concurrency, RefcountBusy) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;

    /* Simulate an open handle holding a reference */
    atomic_store(&vol->health.ref_count, 2);

    hn4_result_t res = hn4_unmount(vol);
    ASSERT_EQ(HN4_ERR_BUSY, res);

    /* Clean up manually since unmount aborted */
    hn4_hal_mem_free(vol->void_bitmap);
    hn4_hal_mem_free(vol->quality_mask);
    hn4_hal_mem_free(vol->nano_cortex);
    hn4_hal_mem_free(vol);
...
/*
 * Test 18.2: RAM Bitmap ECC Corruption (Panic Trigger)
 * RATIONALE:
 * During unmount, the system flushes the "Armored Bitmap" from RAM to Disk.
 * It must verify the ECC of every word before writing. If RAM bitrot is detected,
 * it must NOT persist the corrupted data, must return CPU_INSANITY, and must
 * flag the volume as PANIC/TOXIC to prevent future trust.
 */
hn4_TEST(Integrity, BitmapECCFailure) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;

    /* Ensure we are in a state that triggers flush */
    vol->read_only = false;
    vol->sb.info.state_flags = HN4_VOL_DIRTY;

    /* 
     * Corrupt the RAM bitmap with a deterministic DED pattern.
     * Data = 0. Calculated ECC = 0.
     * Stored ECC = 3 (0000 0011).
     * Diff = 3 (2 bits flipped). Even parity diff = DED = Fatal.
     */
    vol->void_bitmap[0].data = 0ULL;
    vol->void_bitmap[0].ecc  = 0x03; 

    hn4_result_t res = hn4_unmount(vol);

    /* Expect Critical Failure */
    ASSERT_EQ(HN4_ERR_CPU_INSANITY, res);

    /* Clean up manually as unmount likely aborted early or freed vol depending on impl */
    /* If unmount returned error, it might not have freed vol. Check API contract.
       hn4_unmount frees vol on success. On error? 
       Looking at code: teardown happens in Phase 2 regardless of Phase 1 error.
       So vol is freed. We only need to free dev_ptr. */
    
    hn4_hal_mem_free(dev_ptr);
}

/*
 * Test 18.3: Metadata Region Overrun Guard
 * RATIONALE:
 * If the Superblock geometry is corrupted such that the Bitmap Region overlaps
 * or touches the Q-Mask region, the flush loop must detect the boundary violation
 * and truncate the write to prevent overwriting the next region.
 * This sets `persistence_ok = false`, resulting in a Dirty unmount.
 */
hn4_TEST(GeometryLogic, BitmapRegionOverrun) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;
    ...
    /* 
     * Collapse geometry: Start of Q-Mask equals Start of Bitmap.
     * This leaves 0 sectors for the Bitmap. The flush loop logic calculates 
     * `next_lba` and compares against `end_limit`.
     */
    vol->sb.info.lba_qmask_start = vol->sb.info.lba_bitmap_start;

    hn4_result_t res = hn4_unmount(vol);

    /* 
     * Result is OK because unmount completes "safely" (no crash),
     * but the persistence phase was aborted internally.
     */
    ASSERT_EQ(HN4_OK, res);
...
/*
 * Test 18.4: Exabyte Mirror Math (128-bit Safety)
 * RATIONALE:
 * Verify that calculating the location of East (33%) and West (66%) mirrors
 * on a max-capacity (UINT64_MAX) volume does not overflow the intermediate
 * math, which would result in invalid LBAs (e.g. near sector 0).
 */
hn4_TEST(MathSafety, ExabyteMirrorCalculation) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;
    ...
    /* 
     * If the math overflows (e.g. cap * 33 wraps), the target LBA is wrong.
     * If using 128-bit intermediates (or the fixed logic), it works.
     * We assert the function completes without HN4_ERR_GEOMETRY or SIGFPE.
     */
    hn4_result_t res = hn4_unmount(vol);
    ASSERT_EQ(HN4_OK, res);
...
/*
 * Test 18.5: Interrupted Unmount State Recovery
 * RATIONALE:
 * If a volume has the `HN4_VOL_UNMOUNTING` flag set (implying a previous crash
 * during shutdown), it is technically "Clean-ish" but suspicious.
 * The unmount logic should treat this as Dirty (forcing a full flush/sync)
 * and ensure the final state on disk is properly marked HN4_VOL_CLEAN.
 */
hn4_TEST(StateValidation, InterruptedUnmountRecovery) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;

    /* Simulate state found after a crash during unmount */
    vol->sb.info.state_flags = HN4_VOL_CLEAN | HN4_VOL_UNMOUNTING;
    vol->read_only = false;

    /* 
     * The logic inside hn4_unmount (Phase 2 State Analysis) should:
     * 1. Detect UNMOUNTING.
     * 2. Clear CLEAN, Set DIRTY.
     * 3. Perform full flush.
     * 4. Write new CLEAN superblock.
     */
    hn4_result_t res = hn4_unmount(vol);
    ASSERT_EQ(HN4_OK, res);
...
/*
 * Test 18.6: Nano-Cortex Partial Allocation
 * RATIONALE:
 * If `nano_cortex` pointer is valid but `cortex_size` is 0 (or vice versa),
 * the teardown logic must handle the mismatch safely.
 * This simulates a failed mount or memory corruption scenario.
 */
hn4_TEST(ResourceTeardown, CortexSizeMismatch) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;

    /* Pointer valid, size zero */
    ASSERT_TRUE(vol->nano_cortex != NULL);
    vol->cortex_size = 0;
...
/*
 * Test 18.7: Invalid Epoch ID (Zero)
 * RATIONALE:
 * Epoch ID 0 is reserved. If the current epoch ID is 0, the advance logic
 * might misbehave or the next epoch (1) might collide with Genesis.
 * Unmount should handle this gracefully (increment to 1).
 */
hn4_TEST(StateValidation, ZeroEpochIdRecovery) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;
...
/*
 * Test 18.8: Double Unmount Prevention
 * RATIONALE:
 * Calling `hn4_unmount` twice on the same volume handle is a critical
 * Use-After-Free bug. While the API cannot stop the user from passing
 * a dangling pointer, this test verifies that the function nullifies
 * internal pointers before freeing the struct, reducing the blast radius
 * of a UAF if the memory hasn't been reallocated yet.
 * (Note: This is limited by C memory model, but checks "defensive free").
 */
hn4_TEST(Lifecycle, DefensiveFreeChecks) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;

    hn4_result_t res1 = hn4_unmount(vol);
    ASSERT_EQ(HN4_OK, res1);

    /* 
     * We cannot call hn4_unmount(vol) again safely because 'vol' is freed.
     * This test validates the internal implementation details of _safe_release_mem
     * by inspecting the behavior on the auxiliary structures inside the first call.
     * The assertion is that it doesn't crash during the FIRST pass.
     */
...
/*
 * Test 18.9: ZNS Mirror Skipping Logic
 * RATIONALE:
 * On ZNS drives, only Zone 0 (North) is updated. Mirrors are skipped.
 * We verify the return code is OK even though "mirrors_ok" count is 0.
 * The quorum logic must adapt: (ZNS ? North : Quorum).
 */
hn4_TEST(HardwareProfile, ZnsQuorumAdjustment) {
    hn4_volume_t* vol = create_volume_fixture();
    void* dev_ptr = vol->target_device;
    mock_hal_device_t* mdev = (mock_hal_device_t*)dev_ptr;
    ...
    /* 
     * In the mock, writes to mirror LBAs (East/West) will "succeed" 
     * if attempted, but the logic should SKIP them entirely.
     * If logic fails to skip, it might try to write to a Sequential Zone 
     * at a random offset, which the HAL/Drive would reject.
     * Since we can't inspect the skip directly without a spy, we verify success.
     */
    hn4_result_t res = hn4_unmount(vol);
    ASSERT_EQ(HN4_OK, res);
