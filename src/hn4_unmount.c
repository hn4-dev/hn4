...
/* IO wrapper to handle address width differences */
static inline hn4_addr_t _addr_from_sector(uint64_t sect_idx)
{
#ifdef HN4_USE_128BIT
    hn4_addr_t a = { .lo = sect_idx, .hi = 0 };
    return a;
#else
    return sect_idx;
#endif
...
)
{
    ...
    if (vol->taint_counter > 0) {
        cpu_sb.info.dirty_bits |= HN4_DIRTY_BIT_TAINT;
    }
    ...
    if (set_clean && !force_degraded) {
        ...
        cpu_sb.info.state_flags |= HN4_VOL_DIRTY;
        if (force_degraded) {
            cpu_sb.info.state_flags |= HN4_VOL_DEGRADED;
        }
    }
    ...
    for (int i = 0; i < SB_LOC_MAX; i++) {
        ...
        if (io_res == HN4_OK) {
            ...
            if (i == SB_LOC_SOUTH) {
                 ...
                 * If attempt_south is already false, we must not rewind again.
                 */
